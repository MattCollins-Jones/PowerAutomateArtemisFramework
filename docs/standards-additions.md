...
Where the calling system is another Microsoft Entra ID registered app/service and can obtain a token, prefer setting this to Any user in my tenant or Specific users in my tenant over building your own validation logic inside the flow, this is enforced before the flow even starts running, rather than being a Condition you have to remember to add and maintain yourself.
